          ...
          {[
            ...
            <div key={i} className="bg-white border border-[#94B4C1]/60 rounded-xl p-4 sm:p-5 lg:p-6 flex flex-col shadow-sm">
              <div className="flex items-center mb-3 sm:mb-4 gap-3 sm:gap-4">
                <div className="w-10 h-10 sm:w-11 sm:h-11 lg:w-12 lg:h-12 bg-[#84cc16] text-[#001F3F] rounded-lg flex items-center justify-center flex-shrink-0">
                  {feature.icon}
                </div>
                <h3 className="text-lg sm:text-xl lg:text-[20px] text-[#1E488F] font-bold">{feature.title}</h3>
              </div>

              <p className="text-sm sm:text-base lg:text-[16px] leading-6 sm:leading-7 mb-4 flex flex-grow text-[#001F3F] font-medium text-justify">{feature.description}</p>
              
              {/* Button at bottom */}
              <button className="w-full py-2.5 sm:py-3 px-3 sm:px-4 font-semibold bg-[#1E488F] hover:bg-[#1E488F]/90 text-[#F6FCDF] rounded-md border border-[#94B4C1]/30 text-xs sm:text-sm transition flex items-center justify-center">
                {feature.buttonText}
